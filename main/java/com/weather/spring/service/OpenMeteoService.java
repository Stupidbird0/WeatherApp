...
public class OpenMeteoService implements WeatherService {
    ...
    public WeatherData getWeatherByCity(String city) throws Exception {
        ...
        logger.info("🌐 URL chiamata: {}", weatherUrl);
        ...
        WeatherData result = new WeatherData(cityName, temperature, feelsLike, humidity, description, windSpeed);
        
        logger.info("✅ Dati meteo recuperati per {}: {}°C", cityName, temperature);
        
        return result;
    }
    
    /**
     * Metodo per pulire la cache manualmente (opzionale)
     */
    @CacheEvict(value = "weatherCache", key = "#city.toLowerCase()")
    public void evictCityCache(String city) {
        logger.info("🗑️ Cache eliminata per: {}", city);
    }
    
    /**
     * Pulizia automatica della cache ogni ora (opzionale)
     */
    @Scheduled(fixedRate = 3600000) // Ogni ora
    @CacheEvict(value = "weatherCache", allEntries = true)
    public void evictAllCache() {
        logger.info("🗑️ Cache completa svuotata automaticamente");
    }
